  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)] font-display">Coursebooks</h1>
          <p className="text-[var(--text-secondary)] text-sm mt-0.5">{filtered.length} resource{filtered.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      {subjects.length > 0 && (
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setFilter('')}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all ${!filter ? 'bg-indigo-600 text-on-brand' : 'text-[var(--text-secondary)] border border-[var(--border)]'}`}
            style={!filter ? {} : { background: 'var(--surface-700)' }}>All</button>
          {subjects.map(s => (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all ${filter === s ? 'bg-indigo-600 text-on-brand' : 'text-[var(--text-secondary)] border border-[var(--border)]'}`}
      ...
      {showAdd && (
        <div className="card p-5 border-indigo-500/30">
          <h3 className="text-[var(--text-primary)] font-medium mb-4">Add Coursebook</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
            ...
            <div>
              <label className="block text-xs text-[var(--text-secondary)] mb-1">File (optional)</label>
              <input type="file" accept=".pdf,.doc,.docx,.ppt,.pptx"
                className="text-xs text-[var(--text-secondary)] file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-indigo-600 file:text-white hover:file:bg-indigo-700 w-full"
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <p className="text-[var(--text-secondary)]">No coursebooks found</p>
            ...
            <div key={book.id} className="card p-5 flex flex-col gap-3 group hover:border-indigo-500/30 transition-colors">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[var(--text-secondary)]">{book.subject} · {book.class_name}</p>
                </div>
              </div>
              {book.description && <p className="text-xs text-[var(--text-muted)] leading-relaxed">{book.description}</p>}
              <div className="flex items-center gap-2 mt-auto pt-2 border-t border-[var(--border)]">
                {book.file_path && (
                  <a href={`${process.env.NEXT_PUBLIC_API_URL}${book.file_path}`} target="_blank"
                    className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs text-indigo-600 border border-indigo-500/20 hover:bg-indigo-500/10 transition-all">
                    <Download size={11} /> Download
                  </a>
                )}
                <button
                  onClick={() => setAi(a => ({ ...a, open: true, subject: book.subject, class_name: book.class_name }))}
                  className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs text-purple-600 border border-purple-500/20 hover:bg-purple-500/10 transition-all">
                  <Sparkles size={11} /> Plan Lesson
                </button>
                {isTeacher && (
                  <button onClick={() => handleDelete(book.id)}
                    className="ml-auto p-1.5 rounded text-[var(--text-muted)] hover:text-red-600 hover:bg-red-400/10 transition-all">
                    <Trash2 size={14} />
                  </button>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(15,23,42,0.45)' }}>
          <div className="w-full max-w-2xl rounded-2xl border border-[var(--border)] overflow-hidden shadow-2xl" style={{ background: 'var(--surface-850)' }}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
              ...
              <button onClick={() => setAi(a => ({ ...a, open: false, result: '' }))} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] text-xl leading-none">×</button>
            </div>
              ...
              {ai.result && (
                <div className="rounded-xl border border-[var(--border)] p-4 max-h-96 overflow-y-auto" style={{ background: 'var(--surface-900)' }}>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <button className="text-xs text-[var(--text-secondary)] hover:text-[var(--text-primary)]" onClick={() => navigator.clipboard.writeText(ai.result)}>Copy</button>
                  </div>
