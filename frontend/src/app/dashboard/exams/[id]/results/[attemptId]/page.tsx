  ...
  if (!data) {
    return (
      <div className="card p-8 text-center">
        <p className="text-[var(--text-secondary)] font-medium">Could not load exam submission</p>
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center gap-3">
        <Link href={`/dashboard/exams/${id}/results`} className="text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
          <ArrowLeft size={20} />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)] font-display">{data.exam.title} Submission</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="card p-4">
          ...
          <div className="text-sm text-[var(--text-primary)] mt-1">{data.started_at ? new Date(data.started_at).toLocaleString() : '—'}</div>
        </div>
        <div className="card p-4">
          <div className="text-xs text-[var(--text-secondary)]">Submitted</div>
          <div className="text-sm text-[var(--text-primary)] mt-1">{data.submitted_at ? new Date(data.submitted_at).toLocaleString() : 'In progress'}</div>
        </div>
        <div className="card p-4">
          <div className="text-xs text-[var(--text-secondary)]">Tab Switches</div>
          <div className="text-sm text-[var(--text-primary)] mt-1">{data.tab_switches}</div>
        </div>
        <div className="card p-4">
      ...
      {data.termination_reason && (
        <div className="card p-4 border border-red-500/30" style={{ background: 'rgba(239,68,68,0.08)' }}>
          <p className="text-red-600 text-sm font-medium">Termination Reason</p>
          <p className="text-[var(--text-secondary)] text-sm mt-1">{data.termination_reason}</p>
        </div>
      )}

      {data.violations.length > 0 && (
        <div className="card p-5">
          <h2 className="font-semibold text-[var(--text-primary)] mb-3">Violation Log</h2>
          <div className="space-y-2">
            {data.violations.map((violation: any, index: number) => (
              <div key={index} className="rounded-lg p-3" style={{ background: 'var(--surface-700)' }}>
                ...
                <p className="text-[var(--text-secondary)] text-sm mt-1">{violation.details || 'No details provided.'}</p>
                <p className="text-[var(--text-muted)] text-xs mt-1">{violation.timestamp ? new Date(violation.timestamp).toLocaleString() : '—'}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {data.questions.map((question: any, index: number) => (
          <div key={question.id || index} className="card p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs text-[var(--text-secondary)] uppercase tracking-wider">Question {index + 1}</p>
                <h3 className="text-[var(--text-primary)] font-medium mt-1">{question.question}</h3>
              </div>
              <div className="text-right">
                <p className="text-xs text-[var(--text-muted)]">{question.marks} marks</p>
                ...
                {question.is_correct === false && (
                  <span className="inline-flex items-center gap-1 text-red-600 text-xs mt-1">
                    <XCircle size={12} />
                    Incorrect
                  </span>
                )}
              ...
              <div className="mt-4 space-y-2">
                {question.options.map((option: string, optionIndex: number) => {
                  ...
                  return (
                    ...
                    >
                      <span className={isCorrectOption ? 'text-green-600' : isSelected ? 'text-indigo-700' : 'text-[var(--text-secondary)]'}>
                        {option}
                      </span>
                    </div>
                  )
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="rounded-lg p-3" style={{ background: 'var(--surface-700)' }}>
                <p className="text-xs text-[var(--text-secondary)]">Student Answer</p>
                <p className="text-sm text-[var(--text-primary)] mt-1 whitespace-pre-wrap">{question.student_answer || 'No answer submitted.'}</p>
              </div>
              <div className="rounded-lg p-3" style={{ background: 'var(--surface-700)' }}>
                <p className="text-xs text-[var(--text-secondary)]">Correct Answer</p>
                <p className="text-sm text-[var(--text-primary)] mt-1 whitespace-pre-wrap">{question.correct_answer || 'Manual review required.'}</p>
              </div>
            </div>
            ...
            {question.type !== 'mcq' && (
              <div className="mt-4 rounded-lg p-3 border border-indigo-500/20" style={{ background: 'rgba(99,102,241,0.08)' }}>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium text-[var(--text-primary)]">Teacher Grading</p>
                    <p className="text-xs text-[var(--text-secondary)] mt-1">Award marks for this manual-review response.</p>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-sm text-[var(--text-secondary)]">/ {question.marks}</span>
                  </div>
                </div>
              </div>
            )}
