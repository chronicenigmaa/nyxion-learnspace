  ...
  if (!isAdmin) {
    return (
      <div className="rounded-2xl border border-[var(--border)] p-6" style={{ background: 'var(--surface-850)' }}>
        <h1 className="text-xl font-bold text-[var(--text-primary)]">Teacher Profile</h1>
        <p className="mt-2 text-sm text-[var(--text-secondary)]">Only admins can view teacher profiles.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/dashboard/users" className="inline-flex items-center gap-2 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
          <ArrowLeft size={16} />
          Back to Users
        </Link>
      </div>
      ...
      <section className="rounded-2xl border border-[var(--border)] p-5" style={{ background: 'var(--surface-850)' }}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">{data?.teacher.name || 'Teacher Profile'}</h1>
            <p className="mt-1 text-sm text-[var(--text-secondary)]">{data?.teacher.email || 'Loading teacher details...'}</p>
          </div>
      ...
      <section className="rounded-2xl border border-[var(--border)] p-5" style={{ background: 'var(--surface-850)' }}>
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-[var(--text-primary)]">Assigned Sections</h2>
            <p className="text-sm text-[var(--text-secondary)]">Sections where this teacher is the class teacher in EduOS.</p>
          </div>
        </div>

        <div className="flex flex-wrap gap-3">
          {!loading && (!data?.assigned_sections || data.assigned_sections.length === 0) && (
            <div className="text-sm text-[var(--text-muted)]">No assigned sections found.</div>
          )}
          {data?.assigned_sections.map((section) => (
            <div key={section.id} className="rounded-xl border border-[var(--border)] px-4 py-2 text-sm text-[var(--text-primary)]">
      ...
      <section className="rounded-2xl border border-[var(--border)] p-5" style={{ background: 'var(--surface-850)' }}>
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-[var(--text-primary)]">Assigned Students</h2>
            <p className="text-sm text-[var(--text-secondary)]">Students matched from EduOS by this teacher&apos;s assigned class sections.</p>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {!loading && (!data?.assigned_students || data.assigned_students.length === 0) && (
                <tr>
                  <td colSpan={5} className="py-4 text-[var(--text-muted)]">No assigned students found.</td>
                </tr>
              )}
